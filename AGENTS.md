...
- **Agentic Tool System**: 7 tools — `read_file`, `write_file`, `edit_file`, `bash`, `list_files`, `search_files`, `search_content`. Tool call loop with configurable `max_tool_rounds` (default: 10). Two paths: native function calling and text-based JSON parsing fallback.
