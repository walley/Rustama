...
impl App {
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(StreamChunk::Done(stats)) => {
                        ...
                        // Turn finished: persist the committed messages.
    ...
    /// Silently persists the current session under its session name
    /// (autogenerated from the session id unless the user renamed it).
    /// Sessions without a single user message are skipped so merely
    ...
    pub fn autosave_session(&mut self) {
        ...
        match self.save_session() {
            Ok(()) => self.log_event("AUTOSAVE", &format!("session {}", self.session_name)),
            Err(e) => self.log_event("AUTOSAVE_FAIL", &e),
        }
    }

    /// Starts a fresh conversation: autosaves the current session first,
    /// then clears the chat state and mints a new autogenerated session
    /// id/name (the new session is autosaved on its first turn).
    pub fn new_session(&mut self) {
        ...
        self.session_usage = TokenStats::default();
        self.tool_round_count = 0;
        self.tool_call_count = 0;
        self.pending_tool_calls.clear();
        ...
        self.scroll_offset = 0;
        self.set_auto_scroll();
        self.status_message = "New session started".to_string();
        self.log_event("SESSION", "New session started");
    }
    ...
    /// Rows reserved at the bottom of the screen below the input box:
    /// one for the keybar (always shown) plus one for the hint bar when
    /// it is enabled (`hintbar` config key). Mirrors the layout in
    /// `ui()` (main.rs) — keep the two in sync.
    pub fn bottom_rows(&self) -> u16 {
        if self.hintbar { 2 } else { 1 }
    }
    ...
    pub fn handle_click(&mut self, col: u16, row: u16, width: u16, height: u16) {
        ...
        let input_start = height.saturating_sub(6 + self.bottom_rows());
        let input_bottom = height.saturating_sub(2 + self.bottom_rows());
    ...
    fn handle_slash_command(&mut self, input: &str) -> Option<String> {
        ...
        match cmd {
            ...
            "session" => Some(match arg {
                Some("new") => {
                    self.new_session();
                    "Started new session (previous session autosaved)".to_string()
                }
