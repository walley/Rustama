...
| `terminal_send` | Send input (keystrokes) to the running session. Input is written **exactly as given — nothing is appended**; include the Enter key yourself (`\r`) to submit a line, e.g. `ls -la\r`. For TUI apps send bare keystrokes with no newline (`q`, `\u001bOB`…). Backslash escapes are decoded into real control bytes before sending: Enter = `\r`, Ctrl+C = `\u0003`, arrows = `\u001b[A/B/C/D`, Tab = `\t`, Escape = `\u001b`, F-keys = `\u001bOP`…, PgUp/PgDn = `\u001b[5~`/`\u001b[6~` |
